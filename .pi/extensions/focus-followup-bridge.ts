/**
 * Focus live Agent follow-up bridge (protocol v1).
 *
 ...
 *
 * Environment:
 *   FOCUS_AGENT_FOLLOWUP_SOCKET       absolute socket path under the runtime dir
 *   FOCUS_AGENT_FOLLOWUP_TOKEN        random session token
 *   FOCUS_AGENT_FOLLOWUP_RUNTIME_DIR  application-owned private directory (0700)
 */
...
export default function focusFollowupBridge(pi: ExtensionAPI) {
  const socketPathRaw = (process.env.FOCUS_AGENT_FOLLOWUP_SOCKET ?? "").trim();
  const runtimeDirRaw = (process.env.FOCUS_AGENT_FOLLOWUP_RUNTIME_DIR ?? "").trim();
  const token = process.env.FOCUS_AGENT_FOLLOWUP_TOKEN ?? "";
