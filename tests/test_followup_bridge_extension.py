"""Load the real follow-up bridge extension through Pi's jiti loader.
...
from focus.core import FOCUS_PI_FOLLOWUP_EXTENSION_FILE
...
const runtime = mkdtempSync(join(tmpdir(), "focus-bridge-"));
const socketPath = join(runtime, "bridge.sock");
const token = "abcdefghijklmnopqrstuvwxyz0123456789";
process.env.FOCUS_AGENT_FOLLOWUP_SOCKET = socketPath;
process.env.FOCUS_AGENT_FOLLOWUP_RUNTIME_DIR = runtime;
process.env.FOCUS_AGENT_FOLLOWUP_TOKEN = token;
...
def test_followup_bridge_extension_lifecycle() -> None:
    node = shutil.which("node")
    jiti = _find_jiti()
    if not node or jiti is None:
        pytest.skip("node and the Pi jiti loader are required for this check")
    if not FOCUS_PI_FOLLOWUP_EXTENSION_FILE.is_file():
        pytest.skip("follow-up bridge extension is not present")
    harness = _HARNESS.replace("__JITI__", jiti).replace(
        "__EXTENSION__", FOCUS_PI_FOLLOWUP_EXTENSION_FILE.as_uri()
    )
