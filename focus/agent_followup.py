...
def focus_followup_runtime_root() -> Path:
    """Return Focus's private runtime root for follow-up bridge sockets."""
    configured = str(os.environ.get("XDG_RUNTIME_DIR", "") or "").strip()
    if configured:
        root = Path(configured).expanduser()
    else:
        root = Path(tempfile.gettempdir()) / f"focus-{os.getuid()}"
    path = root / "focus" / "agent-followup"
...
def create_followup_runtime(*, generation: int) -> FollowUpRuntime:
    ...
    root = focus_followup_runtime_root()
