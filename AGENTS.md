...
- `focus/summary_editions.py`: strict page-map loader and page renderer for RecordPrep's page-matched summary editions; accepts every supported RecordPrep layout (current v2 and legacy v1 Letter editions) and validates sidecars as a unit (hashes, paths, spans) without repaginating or parsing the PDF layout, exposing the loaded `layout_id` for migration and state decisions. Paginated display text applies a paragraph-spacing transformation (newline runs collapse to exactly one empty line, never at page edges or across paper-page boundaries) for display and paginated search only; stored sidecar text, link offsets, and legacy continuous summaries are unchanged.
