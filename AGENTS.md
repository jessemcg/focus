...
- `focus/summary_editions.py`: strict page-map loader and page renderer for RecordPrep's page-matched summary editions; validates sidecars as a unit (hashes, paths, spans) and never repaginates or parses the PDF layout. Paginated display text applies a paragraph-spacing transformation (newline runs collapse to exactly one empty line, never at page edges or across paper-page boundaries) for display and paginated search only; stored sidecar text, link offsets, and legacy continuous summaries are unchanged.
